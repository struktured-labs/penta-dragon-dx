...
| 0x55BB        | bank-1 | **Sara state machine dispatcher (6 states)** — handler table at 0x55D5 (entries 0x5778, 0x57B6, 0x581A, 0x5869, 0x58D6, 0x592C). All 6 handlers touch DC18 (Sara X), DC19 (Sara state), DC22 (hit-flash), and set sprite-tile IDs (0x54, 0x56, 0x57 — different Sara frames). Reads FFBB/FFBC (input/direction state) and DCF8/DC25 (game state) to choose handler. This is the per-frame Sara animation/movement state machine. |
...
| 0x4F5D        | bank-1 | **Scene-context publisher → D880** — when DD06=0 (no scroll lock) and FFBF=0 (no mini-boss), copies FFB7 (true scene-context byte) into D880 (the sound-engine scene state). This is THE main-loop mechanism that drives D880 transitions during gameplay; the sound engine then follows scene changes. Three-byte fall-through: `LDH A,(FFB7); LD (D880),A; RET`. |
