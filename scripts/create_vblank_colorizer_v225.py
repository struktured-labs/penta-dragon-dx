#!/usr/bin/env python3
"""
v2.25: v1.09 + Crow Palette Fix (STABLE)
...
This brings back v0.99's dynamic palette loading safely combined with
v1.07's stable boss detection and v1.05's tile-based coloring.
"""
import sys
import yaml
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v225.gb")
    ...
    print("\n=== v2.25: Dynamic Boss Palette Loading ===")
    print("  Boss detection via 0xFFBF with DYNAMIC palette swapping:")
    print("    0xFFBF = 1 → Load Gargoyle colors into palette 6")
    print("    0xFFBF = 2 → Load Spider colors into palette 7")
    print("    0xFFBF = 0 → Normal tile-based coloring")
    print()
    ...
    palette_loader_addr = 0x69E0  # Moved to accommodate larger palette loader
    ...
    print(f"Gargoyle palette: 8 bytes at 0x{gargoyle_addr:04X}")
    print(f"Spider palette: 8 bytes at 0x{spider_addr:04X}")
