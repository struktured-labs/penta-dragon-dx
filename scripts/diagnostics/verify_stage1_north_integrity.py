...
def main() -> int:
    ...
    terrain_differences, first_terrain_difference = byte_diff(
        candidate_room[args.dynamic_prefix:],
        baseline_room[args.dynamic_prefix:],
    )
    if first_terrain_difference >= 0:
        first_terrain_difference += args.dynamic_prefix
    gameplay_frame_lag = (
    ...
    if not terrain_ok:
        print(
            ...
            f"{terrain_differences}/{len(candidate_room) - args.dynamic_prefix} "
            "terrain bytes; first offset "
    ...
    print(
        ...
        f"({len(candidate_room) - args.dynamic_prefix} bytes) with "
