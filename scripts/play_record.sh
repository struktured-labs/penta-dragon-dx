#!/bin/bash
# Launch mgba-qt for HUMAN PLAY with auto-recording of (state, action) pairs.
# Output: rl/bc_data/expert_human.jsonl (overwrites; rename to add to dataset)
#
# Usage: ./scripts/play_record.sh [rom_path] [out_path]
#   default ROM:    rom/Penta Dragon (J) [A-fix].gb
#   default OUT:    rl/bc_data/expert_human.jsonl
#
...
OUT="${2:-rl/bc_data/expert_human.jsonl}"
SCRIPT="$PROJECT_DIR/scripts/probes/play_record.lua"
